...
movie_id_current = ratings[0][1]
current_ratings = np.array([])
average_ratings = np.array([])
#for movies uit elkaar halen
for line in range(len(ratings)):
  if (movie_id_current != ratings[line][1] or line == len(ratings)) and line != 0:
    average_ratings=np.append(average_ratings, np.mean(current_ratings))
    current_ratings=np.array([])
    movie_id_current=ratings[line][1]
  else:
    current_ratings=np.append(current_ratings,ratings[line][2])

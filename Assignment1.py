...
#all ratings for movies/items
movie_sorted_ratings = ratings[ratings[:,1].argsort()]
movie_bincount = np.bincount(movie_sorted_ratings[:,1])
movie_cumsum = np.cumsum(movie_bincount)
movie_average_ratings = np.array([])

for i in range(len(movie_bincount)):
  if movie_bincount[i] == 0:
    movie_average_ratings = np.append(movie_average_ratings, 0)
  else:
    movie_current_ratings = movie_sorted_ratings[movie_cumsum[i-1]:movie_cumsum[i],2]
    movie_average_ratings = np.append(movie_average_ratings, np.mean(movie_current_ratings))

#all ratings for users
user_sorted_ratings = ratings[ratings[:,0].argsort()]
user_bincount = np.bincount(user_sorted_ratings[:,0])
user_cumsum = np.cumsum(user_bincount)
user_average_ratings = np.array([])

for i in range(len(user_bincount)):
  if user_bincount[i] == 0:
    user_average_ratings = np.append(user_average_ratings, 0)
  else:
    user_current_ratings = user_sorted_ratings[user_cumsum[i-1]:user_cumsum[i],2]
    user_average_ratings = np.append(user_average_ratings, np.mean(user_current_ratings))
